...
export default function CashFlowManager() {
  ...
  const handleAddOutflow = async () => {
    ...
    try {
      await currentDataService.recordExpense(parseFloat(amount), description, itemsPurchased || undefined);
      await loadCashFlowData();
  ...
  const handleDrawerAdjustment = async () => {
    ...
    try {
      await currentDataService.setCashDrawerBalance(parseFloat(newBalance), adjustmentReason);
      await loadCashFlowData();
  ...
  const getCategoryIcon = (category: string) => {
    switch (category) {
      case 'ORDER_PAYMENT': return <Receipt className="w-4 h-4" />;
      case 'CASH_DEPOSIT': return <PlusCircle className="w-4 h-4" />;
      case 'STOCK_PURCHASE': return <ShoppingCart className="w-4 h-4" />;
      case 'EXPENSE': return <MinusCircle className="w-4 h-4" />;
      case 'REFUND': return <RefreshCw className="w-4 h-4" />;
      case 'CASH_ADJUSTMENT': return <Wallet className="w-4 h-4" />;
  ...
  const getCategoryColor = (category: string, type: string) => {
    ...
    if (type === 'INFLOW') {
      ...
      switch (category) {
        case 'STOCK_PURCHASE': return 'text-blue-600 bg-blue-50 border-blue-200';
        case 'EXPENSE': return 'text-orange-600 bg-orange-50 border-orange-200';
        case 'REFUND': return 'text-red-600 bg-red-50 border-red-200';
      ...
      <div className="p-6">
        <div className="flex items-center justify-center h-64">
          <div className="flex items-center space-x-3">
            ...
            <span className="text-lg font-medium text-gray-600">Loading cash flow data...</span>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 mb-2">Cash Flow Management</h2>
        ...
        <div className="flex items-center space-x-3">
          ...
          <button
            onClick={() => { setModalType('inflow'); setShowAddModal(true); }}
          ...
          <button
            onClick={() => { setModalType('outflow'); setShowExpenseModal(true); }}
      ...
      {summary && (
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          ...
          >
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-blue-600 font-medium group-hover:text-blue-700">Cash Drawer</p>
                <p className="text-2xl font-bold text-blue-900 group-hover:text-blue-800">{formatCurrency(drawerBalance)}</p>
                <p className="text-xs text-blue-500 mt-1">Click to adjust balance</p>
              </div>
          ...
          <div className="bg-gradient-to-br from-green-50 to-green-100 rounded-xl p-6 border border-green-200">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-green-900">{formatCurrency(summary.totalInflow)}</p>
              </div>
          ...
          <div className="bg-gradient-to-br from-red-50 to-red-100 rounded-xl p-6 border border-red-200">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-red-900">{formatCurrency(summary.totalOutflow)}</p>
              </div>
              <TrendingDown className="w-8 h-8 text-red-600" />
            </div>
          </div>

          <div className={`rounded-xl p-6 border ${summary.netFlow >= 0 
            ? 'bg-gradient-to-br from-emerald-50 to-emerald-100 border-emerald-200' 
            : 'bg-gradient-to-br from-orange-50 to-orange-100 border-orange-200'}`}>
            <div className="flex items-center justify-between">
              <div>
                <p className={`text-sm font-medium ${summary.netFlow >= 0 ? 'text-emerald-600' : 'text-orange-600'}`}>
                  Net Flow
                </p>
                <p className={`text-2xl font-bold ${summary.netFlow >= 0 ? 'text-emerald-900' : 'text-orange-900'}`}>
                  {formatCurrency(summary.netFlow)}
                </p>
              </div>
              <DollarSign className={`w-8 h-8 ${summary.netFlow >= 0 ? 'text-emerald-600' : 'text-orange-600'}`} />
            </div>
          </div>
        </div>
      )}
        ...
        <div className="divide-y divide-gray-200">
          {transactions.length === 0 ? (
            <div className="px-6 py-12 text-center">
              ...
              <p className="text-gray-400 text-sm">Start by adding money or recording an expense</p>
            </div>
          ) : (
              ...
              <div key={transaction.id} className="px-6 py-4 hover:bg-gray-50 transition-colors">
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-4">
                    <div className={`p-2 rounded-lg border ${getCategoryColor(transaction.category, transaction.type)}`}>
                    ...
                    <div>
                      ...
                      <div className="flex items-center space-x-4 text-sm text-gray-500">
                        ...
                        <span className="capitalize">{transaction.category.replace('_', ' ').toLowerCase()}</span>
                        {transaction.orderId && (
                          <span className="text-blue-600">Order #{transaction.orderId}</span>
                        )}
                        {transaction.itemsPurchased && (
                          <span className="text-purple-600">Items: {transaction.itemsPurchased}</span>
                        )}
                      </div>
                    </div>
                  </div>
                  <div className="text-right">
                    <p className={`text-lg font-semibold ${transaction.type === 'INFLOW' ? 'text-green-600' : 'text-red-600'}`}>
                      {transaction.type === 'INFLOW' ? '+' : '-'}{formatCurrency(Math.abs(transaction.amount))}
                    </p>
                    <p className="text-sm text-gray-500">by {transaction.createdBy}</p>
                  </div>
                </div>
              </div>
            ))
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-2xl p-8 max-w-md w-full mx-4">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-xl font-bold text-gray-900">Add Money to Cash Drawer</h3>
              <button
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-2xl p-8 max-w-md w-full mx-4">
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Items Purchased (Optional)</label>
                <input
                  type="text"
                ...
                />
                <p className="text-xs text-gray-500 mt-1">Leave empty for general expenses</p>
              </div>
            </div>
      ...
      {showDrawerModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-2xl p-8 max-w-md w-full mx-4">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-xl font-bold text-gray-900">Adjust Cash Drawer Balance</h3>
              <button
            ...
            <div className="space-y-4">
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                ...
                <p className="text-xl font-bold text-blue-900">{formatCurrency(drawerBalance)}</p>
              </div>
              ...
              <div>
                ...
                {newBalance && (
                  <p className={`text-sm mt-2 font-medium ${
                    parseFloat(newBalance) - drawerBalance >= 0 ? 'text-green-600' : 'text-red-600'
                  }`}>
                    Difference: {parseFloat(newBalance) - drawerBalance >= 0 ? '+' : ''}{formatCurrency(parseFloat(newBalance) - drawerBalance)}
                  </p>
                )}
              </div>
              ...
              <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                <p className="text-xs text-yellow-700">
                  <strong>Note:</strong> This adjustment will be recorded as a transaction for audit purposes.
                </p>
              </div>
            </div>
