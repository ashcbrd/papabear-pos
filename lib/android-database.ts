...
class AndroidDatabaseService implements DatabaseService {
  ...
  private async initializeWebStorage(): Promise<void> {
    ...
    if (typeof window !== 'undefined' && window.localStorage) {
      const tables = ['products', 'variants', 'addons', 'ingredients', 'materials', 'orders', 'stock'];
  ...
  async getProducts(): Promise<any[]> {
    ...
    if (Capacitor.isNativePlatform() && this.db) {
      try {
        const result = await this.db.query('SELECT * FROM products ORDER BY createdAt DESC');
    ...
    if (Capacitor.isNativePlatform() && this.db) {
      ...
      if (index !== -1) {
        ...
        if (product.variants) {
          ...
          this.setToWebStorage('papabear_variants', [...filteredVariants, ...newVariants]);
        }
  ...
  async getAddons(): Promise<any[]> {
    ...
    if (Capacitor.isNativePlatform() && this.db) {
      try {
        const result = await this.db.query('SELECT * FROM addons ORDER BY createdAt DESC');
  ...
  async updateAddon(id: string, addon: any): Promise<any> {
    ...
    if (Capacitor.isNativePlatform() && this.db) {
      try {
        ...
        if (addon.stockQuantity !== undefined) {
          const stockResult = await this.db.query('SELECT id FROM stock WHERE addonId = ?', [id]);
  ...
  async getIngredients(): Promise<any[]> {
    ...
    if (Capacitor.isNativePlatform() && this.db) {
      try {
        const result = await this.db.query('SELECT * FROM ingredients ORDER BY createdAt DESC');
        return result.values || [];
      } catch (error) {
  ...
  async createIngredient(ingredient: any): Promise<any> {
    ...
    const id = this.generateId();
    const now = new Date().toISOString();
    const pricePerUnit = ingredient.pricePerUnit || (ingredient.pricePerPurchase / (ingredient.unitsPerPurchase || 1));

    if (Capacitor.isNativePlatform() && this.db) {
      try {
        await this.db.run(
          'INSERT INTO ingredients (id, name, unit, pricePerPurchase, unitsPerPurchase, pricePerUnit, createdAt) VALUES (?, ?, ?, ?, ?, ?, ?)',
          [id, ingredient.name, ingredient.unit, ingredient.pricePerPurchase, ingredient.unitsPerPurchase, pricePerUnit, now]
        );
  ...
  async updateIngredient(id: string, ingredient: any): Promise<any> {
    ...
    const pricePerUnit = ingredient.pricePerPurchase && ingredient.unitsPerPurchase
    ...
    if (Capacitor.isNativePlatform() && this.db) {
      try {
        await this.db.run(
          ...
          [ingredient.name, ingredient.unit, ingredient.pricePerPurchase, ingredient.unitsPerPurchase, pricePerUnit, id]
        );
      ...
      if (index !== -1) {
        ingredients[index] = { ...ingredients[index], ...ingredient, pricePerUnit };
        this.setToWebStorage('papabear_ingredients', ingredients);
  ...
  async getMaterials(): Promise<any[]> {
    ...
    if (Capacitor.isNativePlatform() && this.db) {
      try {
        const result = await this.db.query('SELECT * FROM materials ORDER BY createdAt DESC');
        return result.values || [];
      } catch (error) {
  ...
  async createMaterial(material: any): Promise<any> {
    ...
    const id = this.generateId();
    const now = new Date().toISOString();
    const pricePerPiece = material.isPackage && material.packagePrice && material.unitsPerPackage 
    ...
    if (Capacitor.isNativePlatform() && this.db) {
      try {
        await this.db.run(
          ...
          [id, material.name, material.isPackage ? 1 : 0, material.packagePrice, material.unitsPerPackage, pricePerPiece, now]
        );
    ...
    if (Capacitor.isNativePlatform() && this.db) {
      try {
        await this.db.run(
          ...
          [material.name, material.isPackage ? 1 : 0, material.packagePrice, material.unitsPerPackage, pricePerPiece, id]
        );
  ...
  async getOrders(filters?: any): Promise<any[]> {
    ...
    if (Capacitor.isNativePlatform() && this.db) {
      ...
      if (filters?.filter === 'today') {
        ...
        orders = orders.filter((order: any) => new Date(order.createdAt) >= todayStart);
      }
    ...
    if (Capacitor.isNativePlatform() && this.db) {
      try {
        await this.db.run(
          ...
          [id, orderData.total, orderData.paid, orderData.change, orderData.orderType, orderData.orderStatus || 'QUEUING', JSON.stringify(orderData.items || []), now]
        );
  ...
  async updateOrder(id: string, updates: any): Promise<any> {
    ...
    if (Capacitor.isNativePlatform() && this.db) {
      try {
        ...
        await this.db.run(`UPDATE orders SET ${setClause} WHERE id = ?`, values);
        return { id, ...updates };
      } catch (error) {
  ...
  async updateStock(id: string, quantity: number): Promise<any> {
    ...
    if (Capacitor.isNativePlatform() && this.db) {
      try {
        await this.db.run('UPDATE stock SET quantity = ? WHERE id = ?', [quantity, id]);
        return { id, quantity };
      } catch (error) {
  ...
  async getDashboardStats(filters?: any): Promise<any> {
    ...
    if (Capacitor.isNativePlatform() && this.db) {
      try {
        ...
        const totalResult = await this.db.query('SELECT COUNT(*) as totalOrders, SUM(total) as totalRevenue FROM orders');
