...
class AndroidDatabaseService implements DatabaseService {
  ...
  async getProducts(): Promise<any[]> {
    ...
    if (Capacitor.isNativePlatform() && this.db) {
      try {
        const productsResult = await this.db.query('SELECT * FROM products ORDER BY createdAt DESC');
        ...
        for (const product of products) {
          ...
          for (const variant of variants) {
            ...
            variant.materials = (materialsResult.values || []).map((m: any) => ({
              material: m,
              quantityUsed: m.quantityUsed
            }));
            
            variant.ingredients = (ingredientsResult.values || []).map((i: any) => ({
              ingredient: i,
      ...
      return products.map(product => ({
        ...
        variants: variants
          ...
          .map((variant: any) => ({
            ...variant,
            materials: (variant.materials || []).map((m: any) => ({
            ...
            if (v.ingredients) {
              for (const ingredient of v.ingredients) {
                await this.db.run(
                  ...
                  [this.generateId(), variantId, ingredient.id, ingredient.quantity]
                );
              }
            }
  ...
  async updateIngredient(id: string, ingredient: any): Promise<any> {
    ...
    if (ingredientIndex !== -1) {
      ...
      const pricePerUnit = ingredient.pricePerPurchase && ingredient.unitsPerPurchase
        ...
        : ingredient.pricePerUnit || ingredients[ingredientIndex].pricePerUnit;
        
      ingredients[ingredientIndex] = { ...ingredients[ingredientIndex], ...ingredient, pricePerUnit };
      this.setToWebStorage('papabear_ingredients', ingredients);
  ...
  async updateMaterial(id: string, material: any): Promise<any> {
    ...
    if (materialIndex !== -1) {
      ...
      materials[materialIndex] = { ...materials[materialIndex], ...material, pricePerPiece };
      this.setToWebStorage('papabear_materials', materials);
